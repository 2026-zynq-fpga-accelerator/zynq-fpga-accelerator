# Build a Vitis 2022.2 platform, Stage-3 9-block backbone application, Zynq FSBL, and BOOT.BIN
# from the GAP-engine bitstream/XSA (no new RTL for this round -- same bitstream as the GAP/FC
# release), using firmware sources cloned from Eunsoo Soh's personal repository
# (https://github.com/EunsooSoh/Zynq_FPGA_ResNet, commit 2ca3091) at a location OUTSIDE this team
# repository -- firmware-verification/** in this repo is untouched and this script does not read
# or write anything under it.
#
# stage3_backbone_test_vector.h is gitignored in the firmware repo (regenerated from a Python
# script) and was delivered separately via rtl/build/incoming/; it must already be copied into
# firmware_root/firmware/test/generated/stage3_backbone_test_vector.h before running this script.
#
...
#
# Run with:
#   EUNSOO_FIRMWARE_ROOT=/path/to/clone xsct -nodisp scripts/vitis/build_stage3_backbone_boot_image.tcl all
# or:
#   xsct -nodisp scripts/vitis/build_stage3_backbone_boot_image.tcl all /path/to/clone
#
# Mirrors build_stage3_gapfc_boot_image.tcl's direct arm-none-eabi-gcc compilation approach
# (Vitis's importsources + app build IDE pipeline does not pick up files dropped into imported
# source folders in this headless xsct flow -- see build_stage3_boot_image.tcl's header comment
...
set workspace [file join $repo_root build vitis_stage3_backbone workspace]
...
set boot_dir [file join $repo_root build vitis_stage3_backbone boot]
set platform_name "zybo_resnet_stage3_backbone_platform"
set domain_name "standalone_ps7_cortexa9_0"
set fsbl_domain "standalone_fsbl_ps7_cortexa9_0"
set processor "ps7_cortexa9_0"
set app_name "stage3_backbone_test"
...
if {[llength $argv] < 1 || [llength $argv] > 2 || [lindex $argv 0] ne "all"} {
  error "Usage: EUNSOO_FIRMWARE_ROOT=/path/to/clone xsct -nodisp build_stage3_backbone_boot_image.tcl all\n   or: xsct -nodisp build_stage3_backbone_boot_image.tcl all /path/to/clone"
}
...
foreach required [list $xsa_path $bit_path $cc $bootgen \
    ...
    [file join $firmware_root firmware test stage3_backbone_test.c] \
    [file join $firmware_root firmware test generated stage3_backbone_test_vector.h]] {
...
puts "STAGE3_BACKBONE: workspace=$workspace"
puts "STAGE3_BACKBONE: xsa=$xsa_path"
puts "STAGE3_BACKBONE: firmware(external, not team repo)=$firmware_root"
puts "STAGE3_BACKBONE: processor=$processor"
...
file copy [file join $firmware_root firmware test stage3_backbone_test.c] $test_dir
file copy [file join $firmware_root firmware test generated stage3_backbone_test_vector.h] \
...
set app_sources [list \
  ...
  [file join $test_dir stage3_backbone_test.c] \
...
foreach source $app_sources {
  ...
  puts "STAGE3_BACKBONE: compiling $source"
...
puts "STAGE3_BACKBONE: app ELF built at $app_elf"
...
file mkdir $boot_dir
set bif_path [file join $boot_dir stage3_backbone.bif]
...
puts "STAGE3_BACKBONE_PLATFORM_APPLICATION_FSBL_BUILD_PASS"
puts "STAGE3_BACKBONE_APP_ELF=$app_elf"
puts "STAGE3_BACKBONE_FSBL_ELF=$fsbl_elf"
puts "STAGE3_BACKBONE_BOOT_BIN=$boot_bin"
exit
