# Build a Vitis 2022.2 platform, Stage-3 GAP->FC application, Zynq FSBL, and BOOT.BIN from the
# GAP-engine bitstream/XSA, using firmware sources cloned from Eunsoo Soh's personal repository
# (https://github.com/EunsooSoh/Zynq_FPGA_ResNet, commit 6d5fe98) at a location OUTSIDE this team
# repository -- firmware-verification/** in this repo is untouched and this script does not read
# or write anything under it.
#
# stage3_gapfc_test_vector.h is gitignored in the firmware repo (regenerated from a Python
# script) and was delivered separately via rtl/build/incoming/; it must already be copied into
# firmware_root/firmware/test/generated/stage3_gapfc_test_vector.h before running this script.
#
...
#
# Run with:
#   EUNSOO_FIRMWARE_ROOT=/path/to/clone xsct -nodisp scripts/vitis/build_stage3_gapfc_boot_image.tcl all
# or:
#   xsct -nodisp scripts/vitis/build_stage3_gapfc_boot_image.tcl all /path/to/clone
#
# Mirrors build_stage3_boot_image.tcl's direct arm-none-eabi-gcc compilation approach (Vitis's
...
set workspace [file join $repo_root build vitis_stage3_gapfc workspace]
...
set boot_dir [file join $repo_root build vitis_stage3_gapfc boot]
set platform_name "zybo_resnet_stage3_gapfc_platform"
set domain_name "standalone_ps7_cortexa9_0"
set fsbl_domain "standalone_fsbl_ps7_cortexa9_0"
set processor "ps7_cortexa9_0"
set app_name "stage3_gapfc_test"
...
if {[llength $argv] < 1 || [llength $argv] > 2 || [lindex $argv 0] ne "all"} {
  error "Usage: EUNSOO_FIRMWARE_ROOT=/path/to/clone xsct -nodisp build_stage3_gapfc_boot_image.tcl all\n   or: xsct -nodisp build_stage3_gapfc_boot_image.tcl all /path/to/clone"
}
...
foreach required [list $xsa_path $bit_path $cc $bootgen \
    ...
    [file join $firmware_root firmware test stage3_gapfc_test.c] \
    [file join $firmware_root firmware test generated stage3_gapfc_test_vector.h]] {
...
puts "STAGE3_GAPFC: workspace=$workspace"
puts "STAGE3_GAPFC: xsa=$xsa_path"
puts "STAGE3_GAPFC: firmware(external, not team repo)=$firmware_root"
puts "STAGE3_GAPFC: processor=$processor"
...
file copy [file join $firmware_root firmware test stage3_gapfc_test.c] $test_dir
file copy [file join $firmware_root firmware test generated stage3_gapfc_test_vector.h] \
...
set app_sources [list \
  ...
  [file join $test_dir stage3_gapfc_test.c] \
...
foreach source $app_sources {
  ...
  puts "STAGE3_GAPFC: compiling $source"
...
puts "STAGE3_GAPFC: app ELF built at $app_elf"
...
file mkdir $boot_dir
set bif_path [file join $boot_dir stage3_gapfc.bif]
...
puts "STAGE3_GAPFC_PLATFORM_APPLICATION_FSBL_BUILD_PASS"
puts "STAGE3_GAPFC_APP_ELF=$app_elf"
puts "STAGE3_GAPFC_FSBL_ELF=$fsbl_elf"
puts "STAGE3_GAPFC_BOOT_BIN=$boot_bin"
exit
