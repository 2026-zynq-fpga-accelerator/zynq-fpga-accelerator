#!/usr/bin/env python3
"""Assemble a Stage-3 GAP/FC (OP_GLOBAL_AVG_POOL + FC-as-OP_CONV(kernel=1)) release directory
...
build_stage3_gapfc_boot_image.tcl, then zip it -- same build/releases/<hash>/ + SHA256SUMS.txt +
zip pattern as package_stage2_release.py / package_stage3_release.py.
...
Run with: python3 scripts/release/package_stage3_gapfc_release.py
"""
...
BOOT_DIR = REPO_ROOT / "build" / "vitis_stage3_gapfc" / "boot"
...
def main() -> None:
    ...
    copy_plan = [
        ...
        (BOOT_DIR / "stage3_gapfc.bif", f"boot_image_{git_hash}.bif"),
    ...
    archive = release_dir / f"zybo_stage3_gapfc_{git_hash}_release.zip"
    if archive.exists():
