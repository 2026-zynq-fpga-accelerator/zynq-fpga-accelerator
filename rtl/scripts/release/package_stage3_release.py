#!/usr/bin/env python3
"""Assemble a Stage-3 (R5: 1x1 projection/downsample) release directory
(rtl/build/releases/<git-hash>/) from the artifacts and Vivado reports already produced by
generate_bitstream_xsa.tcl, report_shipped_bitstream_utilization.tcl, and
build_stage3_boot_image.tcl, then zip it -- same build/releases/<hash>/ + SHA256SUMS.txt + zip
pattern as package_stage2_release.py.
...
post_route_*.rpt, which reports the default impl_1 run and can show materially different
numbers (e.g. WNS=-0.614ns/11 failing endpoints there vs the shipped WNS=+0.001ns/0 failing
endpoints here for this build).

Run with: python3 scripts/release/package_stage3_release.py
"""
...
BOOT_DIR = REPO_ROOT / "build" / "vitis_stage3" / "boot"
...
def main() -> None:
    ...
    # name; everything else is suffixed with the git hash, matching the Stage-1/2 convention.
    copy_plan = [
        (BOOT_DIR / "BOOT.BIN", "BOOT.BIN"),
        (BOOT_DIR / "stage3.bif", f"boot_image_{git_hash}.bif"),
    ...
    archive = release_dir / f"zybo_stage3_{git_hash}_release.zip"
    if archive.exists():
